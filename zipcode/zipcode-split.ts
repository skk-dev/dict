...
export function splitAddr2(map: Map<string, string>, kana: string, addr: string) {
  let matches
  if ((matches = addr.match(/^(.+市)(.+区)$/))) {
    ...
    const addrs = [matches[1], matches[2]]
    // 北群馬郡に「ぐん」が含まれる
    if ((matches = kana.match(/^(.+?ぐん(?!まぐん))(.+(?:まち|ちょう|むら|そん))$/))) {
      const kanas = [matches[1], matches[2]]
      setIfSameLength(map, kanas, addrs)
    } else {
      console.error("カナと住所の分割不一致", addrs)
...
export function splitAddr3(map: Map<string, string>, kanaKu: string, addrKu: string) {
  let kana = kanaKu
  let addr = addrKu

  // 区を先に取り除く
  let matches
  if ((matches = addr.match(/^([^−（０-９、）「」〜一二三四五六七八九十]+区(?!画))(.+)$/))) {
    const addrs = [matches[1], matches[2]]
  ...
  const kanaMachiExcludedSuffix = "(?!" + [
    ...
    "い(?:した)?$",
    "ごや$", // 前沢鳥待小屋
    "さきうみほたる$", // 中島地先海ほたる (これ、どこかで切るべきなのか？)
    "だ$", // 原町田, 本町田
    "はらだに$", // 美濃山千原谷
    "ば$",
    "よがみず$", // 山川浜児ケ水
  ...
  if (setIfSameLength(map, kanas, addrs)) {
    ...
    if (kanas.length > addrs.length) {
      const addrSplitter2 = addrJouChoume2 + "|" +
        "(?<=" +
        "^[^−（０-９、）「」〜]+" + addrExcludedPrefix + "町" + addrExcludedSuffix2 + ")|" +
        addrOthers
